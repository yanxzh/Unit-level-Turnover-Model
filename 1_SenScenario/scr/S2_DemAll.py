# -*- coding: utf-8 -*-
"""
Created on Thu Aug 14 14:26:48 2025
...
def dem_get(df_n,df_f):
    ...
    for idem in Dem_scenarios:
        ...
        ind = idem.split('_',)[0]
        iidem = float(idem.split('_',)[1])

        df_this = df_f.copy(deep=True)
        df_this.insert(loc=0,value=idem,column='Dem_scenarios')

        for iyr in yearls:
            final_dem = df_this[yearls[0]].sum()*(1+iidem/100)**(iyr-yearls[0])
            df_this[iyr] = df_this[iyr]*final_dem/df_this[iyr].sum()
...
def inter_main():
    ...
    df_now = df_now.loc[(df_now['Fuel type'].isin(['Total','Other sources'])==0)&\
                        (df_now['Country'].isin(['Rest of the World','World','USA','EU27'])==0),:].reset_index(drop=True)
    ...
    df_fut = pd.read_csv(OUTPUT_PATH+'/S1_DemandTaj/S1_DemandTrajMID.csv')
    ...
    df_dem.to_csv(OUTPUT_PATH+'/S2_DemandSce.csv',index=None)
