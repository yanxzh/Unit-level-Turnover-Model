...
def old_carbon(ieng_sc,iend_sc,ior_sc,isec,Turnover_dir,OldEmis_dir,CCSPP_dir,iyr):
    ...
    if iyr == yearls[1]:
        basic_info = pd.read_csv('../../2_GetPPInfor/output/2_PP_cut/'+isec+'.csv',encoding='utf-8-sig')
        basic_info.drop(['Year','CO2 Eta (%)'],axis=1,inplace=True)
...
def new_carbon_coal(ieng_sc,iend_sc,ior_sc,isec,
    ...
    eff_dict = eff_dict.drop(['Unit'],axis=1).\
                        melt(id_vars=['Country','Tech'],
                             var_name='Year',
                             value_name='Eff')
    eff_dict = eff_dict.loc[eff_dict['Tech']==isec,:]
