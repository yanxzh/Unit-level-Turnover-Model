...
def phaseout_main(ieng_sc,iend_sc,ior_sc,isec,Turnover_dir,CCSPP_dir,iyr):
    ...
    if iyr == yearls[1]:
        pp_file = pd.read_csv('../../2_GetPPInfor/output/2_PP_cut/'+isec+'.csv',encoding='utf-8-sig')
    ...
    production_trend = pd.read_csv('../../1_SenScenario/output/'+dir_prefix+'/S5_OtherTrend.csv')
    ...
    ############################Demo############################
    production_trend = production_trend.loc[production_trend['Country']=='Asia',:]
    ############################Demo############################
