# -*- coding: utf-8 -*-
"""
Created on Mon Oct 13 11:04:12 2025
...
def CCUS_get():
    ...
    df_final = df_final.pivot(index='EnergyScenario',
                              columns='EndScenario',
                              values=str(yearls[yearls.shape[0]-1]))
    df_final = df_final/10**9
    
    df_final.to_csv(OUTPUT_PATH+'/S2_contour/CCUS2050.csv')
...
def sen_plot(sen_data,ax,sec):
    ...
    norm = mcolors.TwoSlopeNorm(vmin=0, vcenter=6, vmax=60)
    ...
    cf = ax.contourf(
        X, Y, Z,
        levels=[zmin, 0.792102,2.15098, zmax],
        colors='none',
        hatches=['', '/////', ''],
        zorder=2,
    )
    ...
    cf = ax.contourf(
        X, Y, Z,
        levels=[zmin, 0.0441269,0.39808, zmax],
        colors='none',
        hatches=['', '/////', ''],
        zorder=2,
    )
    ...
    contour1 = ax.contour(X, Y, Z,levels=[2.21],
                          colors='#8B3E2F',alpha=1,linewidths=2);
    contour2 = ax.contour(X, Y, Z,levels=[1]+list(np.linspace(10,60,6)),
    ...
    cbar.set_label(
        'Annual CCUS deployment in 2050 (Gt/yr)',
    )
    ...
    ax.set_title('Carbon management',font2);
...
if __name__ == '__main__':
    ...
    df_emis = pd.read_csv(OUTPUT_PATH+'/S2_contour/CCUS2050.csv')
    ...
    plt.savefig('../output/sensitive_CCUS.jpg',dpi=100, bbox_inches='tight',format="jpg");
    plt.savefig('../output/sensitive_CCUS.pdf',dpi=50, bbox_inches='tight',format="pdf");
