# -*- coding: utf-8 -*-
"""
Created on Sat Feb  7 10:02:10 2026
...
def CCUS_get():
            ...
            for ior_sc in Order_scenarios:
                for isec in pp_run:
                    df = pd.read_csv('../../3_PPTurnover/output/'+dir_prefix+\
                                     ...
                                     '/8_Summerize/CaptureTrend_'+str(ieng_sc)+'_'+str(iend_sc)+'_'+ior_sc+'_'+isec+'.csv')
                    
                    df.insert(loc=0,value=ieng_sc,column='EnergyScenario')
                    df.insert(loc=0,value=iend_sc,column='EndScenario')
                        
                    df = df.loc[df['Type']=='All',:]
                    df['Total'] = df[yearls.astype(str)].sum(axis=1)
                    df.drop(['Country','Type']+yearls.astype(str).to_list(),axis=1,inplace=True)
    ...
    df_final = df_final.pivot(index='EnergyScenario',
                              columns='EndScenario',
                              values='Total')
    df_final = df_final/10**9
    
    df_final.to_csv(OUTPUT_PATH+'/S2_contour/CCUSTotal2050.csv')
...
def sen_plot(sen_data,ax,sec):
    f = interpolate.interp2d(sen_data.columns.tolist(),
                              ...
                              sen_data.values, kind='linear')#'linear'
    ...
    norm = mcolors.TwoSlopeNorm(vmin=0, vcenter=20, vmax=300)
    con = ax.pcolormesh(X, Y, Z, cmap='Purples', 
                        norm=norm, alpha=0.6, shading='linear')
    con.set_rasterized(True)
    
    contour2 = ax.contour(X, Y, Z,levels=[50,100,150,200,250,300,350],
    ...
    cbar.set_label(
        'Total CCUS deployment by 2050 (Gt)',
    )
    ...
    ax.set_title('Carbon management (2024–2050)',font2);
...
if __name__ == '__main__':
    ...
    df_emis = pd.read_csv(OUTPUT_PATH+'/S2_contour/CCUSTotal2050.csv')
    ...
    plt.savefig('../output/sensitive_CCUS_total.jpg',dpi=100, bbox_inches='tight',format="jpg");
    plt.savefig('../output/sensitive_CCUS_total.pdf',dpi=50, bbox_inches='tight',format="pdf");
